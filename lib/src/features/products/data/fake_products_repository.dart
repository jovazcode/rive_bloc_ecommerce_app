...
final productsRepositoryProvider =
    RiveBlocProvider.finalValue(() => FakeProductsRepository(addDelay: true));
...
final productsListStreamProvider =
    RiveBlocProvider.stream(ProductsListStreamBloc.new);

class ProductsListStreamBloc extends StreamBloc<List<Product>> {
  ProductsListStreamBloc()
      : super((ref, args) =>
            ref.read(productsRepositoryProvider).watchProductsList());
}
...
// }
final productsListFutureProvider = RiveBlocProvider.async(() => AsyncCubit(
    (ref, args) => ref.read(productsRepositoryProvider).fetchProductsList()));
...
// }
final productProvider = RiveBlocProvider.stream(() => StreamBloc(
      (ref, args) {
        final id = args.get<ProductID?>('id', null);
        assert(id != null, 'ProductID is required');
        return ref.read(productsRepositoryProvider).watchProduct(id!);
      },
    ));
...
final productsListSearchProvider =
    RiveBlocProvider.value<ProductsListSearchCubit, Future<List<Product>>>(
        ProductsListSearchCubit.new);

class ProductsListSearchCubit extends ValueCubit<Future<List<Product>>> {
  ProductsListSearchCubit() : super(Future.value([]));

  @override
  Future<List<Product>> build(ref, args) async {
    final query = args?.get<String?>('query', null);
    assert(query != null, 'Query is required');
